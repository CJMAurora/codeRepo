...
public class Graph {
    ...
    public Graph(List<Vertex> vertexs, double[][] edges) {
        this.vertexs = vertexs;
        this.edges = edges;
        initUnVisited();//开始都未被访问
    }
    /*
     * 初始化未访问顶点集合
     */
    ...
    private void updateDistance(Vertex vertex, Vertex neighbor){
        double distance = getDistance(vertex, neighbor) + vertex.getPath();
    ...
    private List<Vertex> getNeighbors(Vertex v) {
        ...
        for (int i = 0; i < vertexs.size(); i++) {
            ...
            if (distance < Double.MAX_VALUE) {
                //是邻居(有路径可达)
                neighbor = getVertex(i);//根据定点位置获取顶点
